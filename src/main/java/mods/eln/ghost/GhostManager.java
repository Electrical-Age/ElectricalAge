...
public class GhostManager extends WorldSavedData {
	...
	Hashtable<Coordinate, GhostElement> ghostTable = new Hashtable<Coordinate, GhostElement>();
	Hashtable<Coordinate, GhostObserver> observerTable = new Hashtable<Coordinate, GhostObserver>();
	...
	public void saveToNBT(NBTTagCompound nbt,int dim) {
		...
		for(GhostElement ghost : ghostTable.values()) {
			if(dim != Integer.MIN_VALUE && ghost.elementCoordinate.dimention != dim) continue;
			NBTTagCompound nbtGhost = new NBTTagCompound();
			ghost.writeToNBT(nbtGhost, "");
			nbt.setTag("n" + nodeCounter++, nbtGhost);
		}		
	...
	public void unload(int dimensionId) {
		...
		while(i.hasNext()) {
			...
			if(n.elementCoordinate.dimention == dimensionId) {
				i.remove();
			}
		}
	}
