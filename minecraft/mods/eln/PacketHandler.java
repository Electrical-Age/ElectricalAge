...
public class PacketHandler implements IPacketHandler {
    ...
    void packetOpenLocalGui(DataInputStream stream,INetworkManager manager, Player player)
    {
    	EntityClientPlayerMP clientPlayer = (EntityClientPlayerMP) player;
    	try {
			clientPlayer.openGui(Eln.instance, stream.readInt(), clientPlayer.worldObj, stream.readInt(), stream.readInt(), stream.readInt());
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    ...
    void packetForNode(DataInputStream stream,INetworkManager manager, Player player)
    {
		try{
    ...
    void packetForClientNode(DataInputStream stream,INetworkManager manager, Player player)
    {
		try{
			EntityClientPlayerMP clientPlayer = (EntityClientPlayerMP) player;
			int x,y,z,dimention;
			...
			if(clientPlayer.dimension == dimention && (node = NodeBlockEntity.getEntity(x, y, z)) != null)
			{
    ...
    void packetNodeSingleSerialized(DataInputStream stream,INetworkManager manager, Player player)
    {
		try{
			EntityClientPlayerMP clientPlayer = (EntityClientPlayerMP) player;
			int x,y,z,dimention;
    ...
    void packetPlayerKey(DataInputStream stream,INetworkManager manager,Player player)
    {
	    	...
	    	{
		    	{
			    	...
			    	if(itemStack != null)
			    	{
				    	if(Utils.hasTheInterface(itemStack.getItem(),IInteract.class))
				    	{
				    		IInteract interactItem = ((IInteract)itemStack.getItem());
    ...
    void packetNodeSerialized24bitPosition(DataInputStream stream,INetworkManager manager, Player player)
    {
		...
		try{
			...
			if(clientPlayer.dimension != dimention) return;
			
			while(stream.available() != 0)
	       	{
				int nx,ny,nz;	
				nx = x + stream.readByte(); 
				ny = y + stream.readByte(); 
				nz = z + stream.readByte(); 
	
				NodeBlockEntity node;
				if((node = NodeBlockEntity.getEntity(nx, ny, nz)) != null)
				{
					if(node.getBlockId() == stream.readShort())
					{
						int streamByteNbrToRead = stream.readByte();
						int streamByteLeft = stream.available();
						node.networkUnserialize(stream);
						if(streamByteLeft-stream.available() != streamByteNbrToRead)
						{
							System.out.println("if(streamByteLeft-stream.available() != streamByteNbrToRead) error");
							while(true);
						}
					}
					else
