...
public class Eln {
	...
	private final static String URL = "http://squareland.ru";
	private final static String LOGO_FILE = "logo.png";
	private final static String[] AUTHORS = { "Radviger" };
	...
	public void load(FMLInitializationEvent event) {
		...
		Item.getItemFromBlock(sixNodeBlock);
	...
	private void registerEnergyConverter() {
		if (ElnToOtherEnergyConverterEnable) {
...

			{
				...
				EnergyConverterElnToOtherDescriptor desc = new EnergyConverterElnToOtherDescriptor(baseName + "LVU", elnDesc);
			...
			}
			{
				...
				EnergyConverterElnToOtherDescriptor desc = new EnergyConverterElnToOtherDescriptor(baseName + "MVU",elnDesc);
			...
			}
			{
				...
				EnergyConverterElnToOtherDescriptor desc = new EnergyConverterElnToOtherDescriptor(baseName + "HVU", elnDesc);
	...
	private void registerTestBlock() {
		/*
		 * testBlock = new TestBlock(); testBlock.setCreativeTab(creativeTab).setBlockName("TestBlock"); GameRegistry.registerBlock(testBlock, "Eln.TestBlock"); TileEntity.addMapping(TestEntity.class, "Eln.TestEntity"); .addName(testBlock,"Test Block"); NodeManager.instance.registerUuid(TestNode.getInfoStatic().getUuid(), TestNode.class);
		 * 
	...
	void registerSolarPannel(int id) {
		...
		new FunctionTableYProtect(new double[] { 0.0, 0.002,
				0.005, 0.01, 0.015, 0.02, 0.025, 0.03, 0.035, 0.04, 0.045,
				0.05, 0.06, 0.07, 0.08, 0.09, 0.10, 0.11, 0.12, 0.13, 1.0 },
				1.0, 0, 1.0);

		new FunctionTable(new double[] { 0.0, 0.1, 0.4, 0.6,
				0.8, 1.0 }, 1);
	...
	void registerRegulatorItem(int id) {
		...
		IRegulatorDescriptor element;
		{
			subId = 0;
			completId = subId + (id << 6);
			element = new RegulatorOnOffDescriptor("OnOFF Regulator 1",
					"onoffregulator", 0.01);
			sharedItem.addElement(completId, element);
		}
		{
			subId = 1;
			completId = subId + (id << 6);
			element = new RegulatorOnOffDescriptor("OnOFF Regulator 10",
					"onoffregulator", 0.1);
			sharedItem.addElement(completId, element);
		}
	...
	void registerMiscItem(int id) {
...

		{
			...
			name = "Wrench";
			GenericItemUsingDamageDescriptorWithComment desc = new GenericItemUsingDamageDescriptorWithComment(
					name, new String[] { Translator.translate("eln.core.tool.wrench.hint0"), Translator.translate("eln.core.tool.wrench.hint1"), Translator.translate("eln.core.tool.wrench.hint2") });
			sharedItem.addElement(subId + (id << 6), desc);
	...
	void recipeRegulatorItem() {
		...
		addRecipe(findItemStack("OnOFF Regulator 10", 1),
				"R R",
				" R ",
				" I ",
				Character.valueOf('R'), new ItemStack(Items.redstone),
		...
		addRecipe(findItemStack("OnOFF Regulator 1", 1),
				"RRR",
				" I ",
				Character.valueOf('R'), new ItemStack(Items.redstone),
				Character.valueOf('I'), new ItemStack(Items.iron_ingot));
